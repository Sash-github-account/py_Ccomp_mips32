...
Version : 2: F
    -> creating separate variables in program object for storing re-used regexs
...
class Program_obj:
    ...
    def expression_syntax_parser(self, var_n_assignmnt_exp):
        return 1
    ...
    def chk_assignment_syntx_n_upd_state(self, line, ln_num):
        re_str_for_var_assignment = "^\s*" + self.re_get_var_name + "\s*" + self.re_get_exp + ";\s*"
        #syntax_match_obj = re.search("^\s*[a-zA-Z_]+[0-9]*[a-zA-Z_]*\s*(=\s*[a-zA-z_\s\(\)\+\-\/\*\^\%0-9]*\s*)?;\s*", line)
        syntax_match_obj = re.search(re_str_for_var_assignment, line)
        print(syntax_match_obj.group())
        if(syntax_match_obj):
            var_n_pure_exp = syntax_match_obj.group().strip(';').strip(' ')
            self.var_declrtn_chkr(var_n_pure_exp, line, ln_num)            
        else:
            self.print_error_msg_ext("ERROR: Syntax error at line: ", line, ln_num)

    def chk_var_re_dec_exp_syntx(self, new_var_lst, line, ln_num):
        for var in new_var_lst:
            if self.var_declrtn_chkr(var):
    ...
    def chk_var_declaration_syntx(self, line, ln_num):
        ...
        var_dec_ln_match_obj = re.search(re_str_to_chk, line)
        if(var_dec_ln_match_obj):
            var_name_only_lst = re.sub("\s*", '', re.sub('(void|int)', '', re.sub(self.re_get_exp, "", var_dec_ln_match_obj.group())).strip("\n").strip(" ").strip(";")).split(',')
            var_name_n_exp = re.sub("\s*", '', re.sub('(void|int)', '', var_dec_ln_match_obj.group()).strip("\n").strip(" ").strip(";")).split(',')
            print("TEST: ", var_name_only_lst)
            if(self.chk_var_re_dec_exp_syntx(var_name_only_lst, line, ln_num)):
                self.var_dec_ln_dict[ln_num] = line
                print("INFO: Found variable declaration at line: " + str(ln_num)+": " + line)
                self.expression_syntax_parser(var_name_n_exp)
                return 1
        else:
            return 1
#FIXME#            #self.print_error_msg_ext("ERROR: Syntax error at line num: ",  line, ln_num)
...
def start_fl_parse(file_lns):
    ...
    for line_num in range(len(file_lns)):
        ...
        elif(prog_obj.chk_assignment_syntx_n_upd_state(file_lns[line_num], line_n)):
            continue
        else:
